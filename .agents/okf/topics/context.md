...
- `packages/cli/src/run.ts:1032` — `resolveRunSandbox()` — the ONE resolver: `--sandbox` (legacy override) > context worker > `inmemory` default
- `packages/cli/src/run.ts:1107` — `runRunCli` wiring — sets `parsed.sandbox = resolveRunSandbox(...)` before BOTH the local (`runTemplate`) and remote (`remoteStartBody`) paths
...
<sub>derived 2026-07-15 · arc=55 commits · files=3 · lessons=35</sub>
<!-- okf:auto-end -->
