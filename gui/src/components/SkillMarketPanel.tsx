...
import { loadSkills, type MarketSkill } from "../data/runView";
import "../styles/skillmarket.css";

type Ring = MarketSkill["ring"];
...
export function SkillMarketPanel({ activeRun, open, onClose }: { activeRun: string; open: boolean; onClose: () => void }) {
  ...
  const shown = useMemo(() => marketFilter(skills ?? [], query, ring ?? undefined), [skills, query, ring]);
