  ...
  $defs: {
    ...
    op: {
      ...
      properties: {
        ...
        // (op-integrity §1/§2) Deterministic post-conditions + the structured-verdict pointer. SIMPLICITY LADDER:
        // the DEFAULT integrity check is the simplest sufficient one — file-exists/non-empty/min-bytes ("did the
        // staged file exist and get content"). Escalate to json-parses/json-schema only for a structured result, and
        // to json-pointer/contains-marker ONLY for a load-bearing field (a pointer/marker couples the flow to a file
        // internal — never for decoration). The MANIFEST convention (an op emits a JSON resultFile it then asserts
        // over) is how you stay cheap when a structured field genuinely must be checked — not "always assert content".
        expect: {
          type: 'array',
          description: 'Integrity post-conditions checked AFTER this op runs, over its writes. Default consequence warn (set onFailure:block to block). Reuses the check predicates; programmatic-only. DEFAULT to file-exists/min-bytes; use json-parses/json-schema for structured results and json-pointer/contains-marker ONLY for a load-bearing field.',
          items: { $ref: '#/$defs/integrityExpectation' },
        },
        resultFile: { type: 'string', minLength: 1, description: "Run-relative path to this op's STRUCTURED verdict — by convention a JSON manifest. On failure the runner builds the op-failure detail from THIS file (not stderr), and json-schema/json-pointer `expect`s can target it when a structured field is load-bearing." },
    ...
    integrityExpectation: {
      ...
      // aliases the check predicates. SIMPLICITY LADDER (default → escalate only as the contract demands):
      // (0) op exit-code → (1) file-exists/non-empty/min-bytes DEFAULT → (2) json-parses/json-schema for a structured
      // result → (3) json-pointer-*/contains-marker for a LOAD-BEARING field only (contains-marker last, opaque prose).
      type: 'object',
      additionalProperties: false,
      required: ['kind'],
      properties: {
        kind: {
          type: 'string',
          minLength: 1,
          description: 'file-exists | min-bytes | contains-marker | json-parses | json-pointer-exists | json-pointer-equals | json-schema. DEFAULT to file-exists/min-bytes; escalate to json-*/pointer only as the contract demands.',
        },
