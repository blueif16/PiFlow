...
//
// PROJECT-LOCAL: a temp WORKSPACE (product root) carries the `.agents/skills/<id>/SKILL.md` bundles the wire
// resolves against — the global `~/.piflow/skills` is never read (mirrors skill-locate.test.ts).
...
let WS: string; // workspace = product root (both skill rings hang off it)
...
const ctx = () => ({ run: path.join(WS, 'out', 'r1'), workspace: WS });
