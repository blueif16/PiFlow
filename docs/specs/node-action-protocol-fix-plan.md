...
**STEPS**
- New `packages/core/src/workflow/reroute/expand.ts` — `expandReroute(spec)`, structured like `expandFusion` (returns spec unchanged when no node has `reroute`; throws `RerouteConfigError` on a non-ancestor `onFail` or `max < 1`).
...
- Insert in BOTH entries AFTER `expandFusion`: `runFromConfig` after `entry.ts:71`; `runFromTemplate` after `entry.ts:109`. Order: `profile → expandFusion → expandReroute → compile`. (`entry.ts:107` = `applyProfileByName`; not a site.)
...
**Closes:** #4 (IN FULL — escalate-with-evidence is the #4 core), #6, #15 · **G-number:** G12 · **Branch:** `feat/g12-trigger-actions`
...
**ACCEPTANCE:** #4 escalate-with-evidence ported IN FULL; #6 retry on a quality verdict; #15 `stop` wired as the documented `block` alias (no additivity-breaking abort); deterministic model-free tests (no live pi).
