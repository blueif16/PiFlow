...
| `local` (real run) | **KERNEL-ENFORCED on darwin** (deny-reads → allow scope+toolchain) | **KERNEL-ENFORCED on darwin** (deny-writes → allow workdir+`owns`+scratch) | open | **EPERM** (read AND write) on darwin; open on Linux (warns) | `sandbox/local.ts` `exec` → `seatbeltExecPlan` |
